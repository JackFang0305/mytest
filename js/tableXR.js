...
var Table = function () {
    ...
    _createClass(Table, [{
        ...
        value: function tableHtml() {
            ...
            var container = ['<div class="Table">', '<div class="T-title"style="overflow: auto;">', '<div>' + title + '</div>', '</div>', '<div class="T-content">', '<label for="T-sort">', '\u6BCF\u9875\u663E\u793A\u6761\u6570\uFF1A<select name="" id="T-pageNum">'].concat(_toConsumableArray([5, 10, 150].map(function (c) {
                return '<option value="' + c + '">' + c + '</option>';
            })), ['</select>', '\u6392\u5E8F\uFF1A<select name="" id="T-sort">'], _toConsumableArray(columns.map(function (c) {
                return '<option value="' + c.data + '">' + c.title + '</option>';
            })), ['</select><input style="border-bottom:1px solid #e1e1e8;border-left: 1px solid #e1e1e8; padding: 5px 10px; width:800px;height:21px; background-color:yellow" type="text" name="T-search" placeholder="\u641C\u7D22"></label>', '<table border="0" cellspacing="0" cellpadding="0">'], _container, ['</table>', '<div id="T-page"></div>', '</div>', '</div>']).join('');
